...
_ME.entry = (info) => {
	_ME.replaceInTempaltes(/CRN_ENTRY_MODULE_NAME/g, info.name);
};

_ME.main = (info) => {
	_ME.replaceInTempaltes(/CRN_REGISTER_COMPONENT_NAME/g, info.registerComponentName);
};
...
_ME.save = (info) => {
	if (OPTIONS.bundle) {
		...
		// 写入文件。
		fs.writeFileSync(exporter.name2realpath(info.name), info.code);
	}
};
...
_ME.cmd2amd = (info) => {
	...
	var transformer = new UglifyJS.TreeTransformer(function(node, descend) {
		...
		// 捕捉所有的 require() 调用。
			...
			// 公包预加载模块，替换为公包中的模块名。
			let definedName = OPTIONS.COMMON_MODULES[requireName];
			if (definedName) {
				return new UglifyJS.AST_Call({
					expression: new UglifyJS.AST_SymbolRef({ name: 'require' }),
					args: [ new UglifyJS.AST_String({ value: definedName }) ]
				});
			}
	...
	var ast = UglifyJS.parse(info.code);
...
_ME.bundle = (() => {
	var run = (callback) => {
		...
		fs.writeFileSync(OPTIONS.bundle, code);
...
_ME.run = () => {
	...
	// 输出入口文件。
		...
		fs.writeFileSync(exporter.name2realpath(infoEntry.name), infoEntry.code);
